...
def trackbox_newest(request, page_number):
	v = Video.objects.all().order_by('-upload_date')[(page_number-1)*17:(page_number*17)-1]
	return render_to_response('trackbox.html', {'videos': v})
	
def trackbox_popular(request, page_number):
	v = Video.objects.all().order_by('-upload_date').order_by('-num_likes')[(page_number-1)*17:(page_number*17)-1]
	return render_to_response('trackbox.html', {'videos': v})
...
def trackbox_my_library(request, page_number):
	...
	v = l.video_set().all().order_by('-upload_date')[(page_number-1)*17:(page_number*17)-1]
	return render_to_response('trackbox.html', {'videos': v})